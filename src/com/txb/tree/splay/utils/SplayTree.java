package com.txb.tree.splay.utils;
	...
	private SplayTreeNode<T> splay(SplayTreeNode<T> tree, T key) {
		...
		for (;;) {
			...
			if (cmp < 0) {
				...
				//无左子树，此时已经找到所要的元素，就是现在的根元素
				if (tree.left == null)
					break;

				//在左子树，且所要找的元素在左子树的左子树，此时要把根节点的左子树节点变为根节点
				if (key.compareTo(tree.left.key) < 0) {
					c = tree.left;                           /* rotate right */
					tree.left = c.right;
					c.right = tree;
					tree = c;//c是要变为根节点的节点
					if (tree.left == null) //如果此时根节点无左子树，说明循环终止
						break;
				}
				
				//此初对应此时根节点有左子树，tree为下一个要成为根节点的节点
				r.left = tree;                               /* link right */
				r = tree;
				tree = tree.left;
			} else if (cmp > 0) {//假如所要找的元素在右子树
